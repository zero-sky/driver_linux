...
{
    // 在模块初始化时，globalmem_dev->cdev的地址被挂载到了inode->i_cdev上面
    // 通过下面的函数，可以通过i_cdev的地址获取到globalmem_dev的地址
...
static int __init globalmem_init(void)
{
    ...
    if(globalmen_major) {
        // 注册一个设备号的范围，从globalmem_major,0到globalmem_major, DEVICE_NUM
    ...
    // 申请一个空间用于存放设备的特定信息
    globalmen_devp = kzalloc(sizeof(struct globalmem_dev) * DEVICE_NUM, GFP_KERNEL);
    if(!globalmem_devp) {
        ret = -ENOMEM;
        goto fail_malloc;
    }
